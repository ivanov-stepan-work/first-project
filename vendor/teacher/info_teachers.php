...
$id_department = $connect->query("SELECT `departments`.`id_department` FROM `departments` WHERE `departments`.`head_id` = " . $id_user)->fetchColumn();
...
WHERE projects.id_project IS NULL AND `groups`.`dep_id` = '$id_department'
...
FROM
    `projects` AS P
INNER JOIN `department_teachers` AS DT
ON
    DT.`teacher_id` = P.`teacher_id`
INNER JOIN `departments` AS D
ON
    D.`id_department` = DT.`department_id`
WHERE
    D.`head_id` = '" . $_SESSION['user']['id'] . "'
) AS projects,
(
SELECT
    SUM(NoS.`numbers_of`)
FROM
    `number_of_students` AS NoS
INNER JOIN `department_teachers` AS DT
ON
    DT.`teacher_id` = NoS.`teacher_id`
INNER JOIN `departments` AS D
ON
    D.`id_department` = DT.`department_id`
WHERE D.`head_id` = '" . $_SESSION['user']['id'] . "'
) AS students;
";
...
AND N.`numbers_of` <> 0
AND D.`head_id` = '" . $_SESSION['user']['id'] . "'
        ...
        <div class="accordion d-flex flex-column align-items-center">
            <p class="d-flex flex-column w-25 align-items-center justify-content-center">
                ...
                <button class="btn btn-primary mx-4 mb-4 w-100" type="button" data-bs-toggle="collapse"
                    ...
                    Отчет по студентам без проектов
                </button>
            </p>
            <div class="collapse w-50 text-center" id="SWPStats" data-bs-parent=".accordion">
                <h2 class="text-center">Отчет по студентам без проектов</h2>
